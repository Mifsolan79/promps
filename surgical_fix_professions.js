...
"[DETECTIVE PRIVADO] **Gabardina clásica, sombrero fedora, office en penumbra con persianas venecianas, humo de cigarrillo y luz de neón exterior.**",
...
"[GEÓGRAFO CARTÓGRAFO] **Inclinado sobre mapa topográfico gigante, brújula y escalímetro, oficina con globos terráqueos antiguos.**",
...
// Buscamos el final de la última categoría válida antes de la corrupción
// La corrupción parece empezar después de la última categoría cerrada antes de PROFESIONES
// Buscamos la última ocurrencia de de un bloque cerrado que no sea PROFESIONES
// O más simple: truncamos el archivo en la primera ocurrencia de "PROFESIONES" (que era la última agregada)

const startOfCorruption = content.indexOf('\"PROFESIONES\":');
if (startOfCorruption !== -1) {
    // Truncamos justo antes de la categoría corrupta
    let cleanContent = content.substring(0, startOfCorruption);
    
    // Limpiamos comas o espacios finales para que el objeto quede bien
    cleanContent = cleanContent.trim().replace(/,$/, '');
    
    // Añadimos la categoría limpia
    const newProfSection = `,\n    \"PROFESIONES\": ` + JSON.stringify(formattedProfessions, null, 4) + `\n};`;
    
    fs.writeFileSync(filePath, cleanContent + newProfSection);
    console.log('Base de datos regenerada quirúrgicamente.');
} else {
    console.error('No se encontró el inicio de la categoría PROFESIONES.');
}
